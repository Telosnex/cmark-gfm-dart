...
/// Forked from package autolink_postprocess.dart for CmarkNode2/CmarkIter2.
void applyAutolinks2(CmarkNode2 root) {
  _mergeAdjacentTextNodes(root);
  final iter = CmarkIter2(root);
  var depthInLink = 0;
...
bool _isSuppressedContext(CmarkNode2 node) {
...
void _processTextNode(CmarkNode2 node) {
  final literal = node.contentString;
  if (literal.isEmpty) {
    return;
  }
...
void _replaceTextWithMatches(
  ...
  for (final match in matches) {
    ...
    if (spanStart > last) {
      ...
      if (text.isNotEmpty) {
        final textNode = CmarkNode2(CmarkNodeType.text)..setLiteral(text);
    ...
    if (url.isNotEmpty) {
      final link = CmarkNode2(CmarkNodeType.link)
        ..linkData.url = url
        ..linkData.title = '';
      final textNode = CmarkNode2(CmarkNodeType.text)..setLiteral(match.text);
  ...
  if (last < data.length) {
    ...
    if (trailing.isNotEmpty) {
      final textNode = CmarkNode2(CmarkNodeType.text)..setLiteral(trailing);
      parent.insertAfter(cursor, textNode);
      cursor = textNode;
    }
  }
...
void _mergeAdjacentTextNodes(CmarkNode2 root) {
  final iter = CmarkIter2(root);
  while (true) {
    final event = iter.next();
    if (event == CmarkEventType.done) {
    ...
    while (child != null) {
      if (child.type == CmarkNodeType.text) {
        ...
        while (next != null && next.type == CmarkNodeType.text) {
          child.content.write(next.contentString);
          final toRemove = next;
          next = next.next;
          toRemove.unlink();
        }
...
void _trimAdjacentAngleBrackets(CmarkNode2 link) {
  final prev = link.previous;
  if (prev != null && prev.type == CmarkNodeType.text) {
    final text = prev.contentString;
    if (text.endsWith('<')) {
